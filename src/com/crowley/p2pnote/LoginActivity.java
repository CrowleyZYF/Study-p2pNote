...
public class LoginActivity extends Activity implements OnClickListener {
	...
	private Handler handler = new Handler(){
		public void handleMessage(Message msg){
			if(msg.what==0x123){
				...
				case NOT_EXIST:{
					Common.errorDialog(nowContext, "登陆失败", "不存在该账号！").show();
					break;
				}
				case PASS_ERROR:{
	...
	public void login(String account,String password){
		if(!Common.isOpenNetwork(this)){
			Common.errorDialog(nowContext, "登陆失败", "网络未链接").show();
		}else if(account==""){
			Common.errorDialog(nowContext, "登陆失败", "账号不得为空").show();
		}else if(password==""){
			Common.errorDialog(nowContext, "登陆失败", "密码不得为空").show();
		}else if(!returnList.isEmail(account)){
			Common.errorDialog(nowContext, "登陆失败", "邮箱格式有误！").show();					
		}else {
